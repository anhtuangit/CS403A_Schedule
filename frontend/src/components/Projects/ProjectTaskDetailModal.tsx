        ...
        <div className="p-6">
          ...
          <div className="space-y-6">
            ...
            {currentTask.subtasks && currentTask.subtasks.length > 0 && (
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Task con ({currentTask.subtasks.filter(s => s.completed).length}/{currentTask.subtasks.length} hoàn thành)
                </label>
          ...
          <div className="flex justify-end gap-4 pt-6 mt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <button
              onClick={onEdit}
              disabled={!canEdit || !onEdit}
              className={`btn ${canEdit && onEdit ? 'btn-primary' : 'btn-secondary opacity-50 cursor-not-allowed'}`}
            >
